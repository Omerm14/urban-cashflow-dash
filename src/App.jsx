...
// ── Sidebar ───────────────────────────────────────────────────────────────────
function Sidebar({ view, setView, suppliersCount, onUpgrade, onUpload, mobileOpen, setMobileOpen, plan, planUsed, planLimit, planPct, user, onSignOut, onStartTour }) {
  ...
  const navItem = (id, label, Icon, badge, badgeRed, obKey) => {
    const active = view === id;
    return (
      <button key={id} data-onboarding={obKey || undefined} onClick={() => navigate(id)} style={{ display: "flex", alignItems: "center", gap: 9, width: "100%", padding: "8px 10px", borderRadius: 7, border: "none", cursor: "pointer", background: active ? "rgba(99,102,241,.10)" : "transparent", boxShadow: active ? "inset 3px 0 0 #6366F1" : "none", color: active ? "#CBD6E6" : SB.t2, fontFamily: SANS, fontSize: 13, fontWeight: active ? 600 : 400, transition: "background .12s" }}
  ...
  const MAIN = [
    ...
    { id: "integrations", label: t("nav_integrations"), Icon: Zap,            obKey: "integrations" },
  ];
      ...
      <div style={{ width: 240, background: SB.bg, display: "flex", flexDirection: "column", flexShrink: 0, height: "100vh", overflowY: "auto", ...(isDrawer ? { position: "fixed", top: 0, left: 0, zIndex: 200, animation: "slideRight 0.25s ease" } : { position: "sticky", top: 0 }) }}>
        ...
        <div style={{ padding: "0 8px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
            {MAIN.map(({ id, label, Icon, obKey }) => navItem(id, label, Icon, undefined, undefined, obKey))}
          </div>
        </div>
        ...
        <div style={{ padding: "0 8px" }}>
          ...
          {navItem("suppliers", t("nav_suppliers"), Users, suppliersCount, undefined, "suppliers")}
        ...
        <div style={{ padding: "12px" }}>
          ...
          {onStartTour && (
            <button onClick={onStartTour} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 6, background: "transparent", border: "none", cursor: "pointer", marginTop: 2 }}
              onMouseEnter={e => e.currentTarget.style.background = "rgba(99,102,241,.08)"}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(165,180,252,.6)" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polygon points="10 8 16 12 10 16 10 8"/></svg>
              <span style={{ fontFamily: SANS, fontSize: 12, color: "rgba(165,180,252,.6)", fontWeight: 500 }}>Take the tour</span>
            </button>
          )}
...
function Dashboard({ invoices, onPayAllJuly, chartData, supplierNames, supplierColor, user, onMissingAlert, onAnomalyAlert, missingSuppliers, anomalyMap, onViewMonth, onNavigateFiltered }) {
  ...
  return (
    <div style={{ animation: "slideUp 0.35s cubic-bezier(.16,1,.3,1)" }}>
      ...
      <div data-onboarding="kpi" style={{ display: "grid", gridTemplateColumns: kpiCols, gap: 12, marginBottom: 16 }}>
...
export default function App() {
  ...
  // Onboarding
  const { phase, tourStep, checklistVisible, checklistAllDone, tasks, beginTour, skipWelcome, finishTour, skipTour, nextStep, prevStep, dismissChecklist, startFromWelcome } = useOnboarding({ userId: user?.id, invoices, suppliers });
    ...
    <ThemeCtx.Provider value={T}>
      <LayoutCtx.Provider value={{ isMobile, isTablet }}>
        ...
        <div style={{ display: "flex", minHeight: "100vh", background: T.bg, fontFamily: SANS, color: T.t1, direction: lang === "he" ? "rtl" : "ltr" }}>
          ...
          {phase === 'welcome'   && <OnboardingWelcome onBegin={beginTour} onSkip={skipWelcome} />}
          {phase === 'tour'      && <OnboardingTour step={tourStep} onNext={nextStep} onPrev={prevStep} onSkip={skipTour} onFinish={finishTour} />}
          {phase === 'celebrate' && <OnboardingCelebration />}
          {(checklistVisible || checklistAllDone) && view === 'dashboard' && (
            <OnboardingChecklist tasks={tasks} onDismiss={dismissChecklist} onRestartTour={startFromWelcome} />
          )}
        </div>
        </LangCtx.Provider>
      </LayoutCtx.Provider>

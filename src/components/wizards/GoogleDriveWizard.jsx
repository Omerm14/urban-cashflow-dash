  ...
  const handleConnect = async () => {
    ...
    try {
      const resp = await fetch(
        `/api/integrations/google/auth-url?type=google_drive&returnUrl=${encodeURIComponent(window.location.origin)}`,
  ...
  return (
    <div>
      <p style={{ color:'#94a3b8', fontSize:13, lineHeight:1.7, marginBottom:24 }}>
        Connect your Google account to let Cashflow scan your Drive for PDF and image invoices and import them automatically.
      </p>
  ...
  return (
    <div>
      <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
        <div>
          <label style={{ fontSize:12, color:'#94a3b8', fontWeight:500, display:'block', marginBottom:6 }}>Folder ID <span style={{ color:'#475569' }}>(optional)</span></label>
          <input
            className="input"
            placeholder="Leave blank to scan all of Drive"
            value={config.folder_id}
            onChange={e => setConfig(c => ({ ...c, folder_id: e.target.value }))}
          />
          <div style={{ fontSize:11, color:'#334155', marginTop:5, lineHeight:1.5 }}>
            Find your folder ID in the Drive URL after <code style={{ color:'#a78bfa' }}>/folders/</code>
          </div>
        </div>
        <div>
          <label style={{ fontSize:12, color:'#94a3b8', fontWeight:500, display:'block', marginBottom:8 }}>File types to import</label>
          <SegmentedPicker
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 16px', background:'#0f172a', borderRadius:10, border:'1px solid #1e2d45' }}>
          <div>
            ...
            <div style={{ fontSize:11, color:'#64748b', marginTop:3 }}>Automatically import new invoices on schedule</div>
          </div>
  ...
  return (
    <div>
      <div style={{ background:'#0f172a', border:'1px solid #1e2d45', borderRadius:12, padding:'4px 20px', marginBottom:20 }}>
        <ReviewRow label="Folder" value={config.folder_id || 'All of Drive'} />
...
export default function GoogleDriveWizard({ mode, integration: initialIntegration, initialStep, onClose }) {
  const [integration] = useState(initialIntegration);
  const [config, setConfig] = useState({
    folder_id:      initialIntegration?.config?.folder_id      || '',
  ...
  return (
    <WizardShell stepLabels={STEP_LABELS} title="Google Drive" onClose={onClose} startIndex={initialStep || 0}>
      <ConnectStep />

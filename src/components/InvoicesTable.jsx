...
const SortArrow = ({ field, sortField, sortDir }) => {
  if (sortField !== field) return <span style={{ color: 'var(--t3)', marginLeft: 3, fontSize: 9 }}>⇅</span>;
...
export default function InvoicesTable({ computed, dupeIds, updateInvoice, deleteInvoice, setEditInvoice, color, selectedIds, onToggleSelect, onToggleAll, onViewAttachment, sortField, sortDir, onSort }) {
  ...
  return (
    <div className="card" style={{ padding:0, overflow:"hidden" }}>
              ...
              <tr key={inv.id} className={isSelected ? "sel" : ""}>
                ...
                <td>
                  <div style={{ display:"flex", gap:6, justifyContent:"flex-end" }}>
                    {inv.attachment_path && (
                      <button title="View original file" onClick={() => onViewAttachment?.(inv)} style={{ width:26, height:26, border:'1px solid var(--bdr)', borderRadius:6, background:'transparent', cursor:'pointer', display:'inline-flex', alignItems:'center', justifyContent:'center' }}>
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--t3)" strokeWidth="2" strokeLinecap="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
                      </button>
                    )}
                    {inv.status !== STATUS.PAID && (
                      <button className="btn btn-success btn-sm" onClick={() => updateInvoice(inv.id, { status: STATUS.PAID })}>✓ Paid</button>
                    )}
                    <button title="Edit" onClick={() => setEditInvoice({...inv})} style={{ width:26, height:26, border:'1px solid var(--bdr)', borderRadius:6, background:'transparent', cursor:'pointer', display:'inline-flex', alignItems:'center', justifyContent:'center' }}>
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--t3)" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
      ...
      <div style={{ padding:"12px 18px", display:'flex', alignItems:'center', background:'var(--surf2)', borderTop:"1px solid var(--bdr)" }}>
        <button className="btn btn-ghost btn-sm"
          onClick={() => setEditInvoice({ id:null, supplier:"", invoiceNo:"", invoiceDate:"", amount:"", dueDate:"", status:STATUS.UNPAID, notes:"" })}>
          + Add Manually
        </button>
        <div style={{ flex:1 }}/>
        <span style={{ fontSize:12, color:'var(--t2)', fontWeight:500 }}>{computed.length} invoice{computed.length !== 1 ? 's' : ''}</span>
      </div>
    </div>
  );
}

...
describe('required env vars (CASH-35)', () => {
  ...
  // 2026-07-19 incident regression test: FRONTEND_URL was briefly included
  // here, which took production down (FUNCTION_INVOCATION_FAILED on every
  // /api/* request) because Vercel Production never had it configured, even
  // though every call site that reads it already falls back to a default.
  it('does not require FRONTEND_URL — every call site already falls back when it is unset', () => {

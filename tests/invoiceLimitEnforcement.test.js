...
describe('uploadAttachment (invoices.js: manual browser upload also hard-blocks over quota)', () => {
  it('returns 402 and never touches the multipart body when over limit', async () => {
    require.cache[supabasePath] = {
      id: supabasePath, filename: supabasePath, loaded: true,
    ...
    const res = makeRes();
    // The quota check runs before any busboy/multipart parsing, so a minimal
    // req (no body/headers needed for busboy) is enough to prove the 402
    // short-circuits before ever reaching req.pipe(bb).
    await invoicesRoute.uploadAttachment({ user: { id: 'user1' } }, res);
